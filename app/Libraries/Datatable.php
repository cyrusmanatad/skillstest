...
class Datatable {
	...
	public static function filter ( $request, $columns, &$bindings, ?int $userId = null )
	{
		...
		if ( $where !== '' ) {
			...
			$where = $userId ? $where . ' AND user_id = ' . $userId : $where;
		}elseif($userId) {
	...
	public static function simple ( $request, $conn, $table, $primaryKey, $columns, $userId = null )
	{
		...
		$where = self::filter( $request, $columns, $bindings, $userId );
	 ...
	 *
	 * @param  object|resource $db  Database handler
	 * @param  string|array    $bindings Array of PDO binding values from bind() to be
